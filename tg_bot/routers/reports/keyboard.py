    ...
    for day in range(1, days_in_month + 1):
        ...
        if start_date <= current_date <= end_date:
            calendar_days.append(InlineKeyboardButton(
                text=bold_numbers(day), callback_data=f'day:{year}:{month}:{day}'
            ))
        else:
            calendar_days.append(InlineKeyboardButton(
                text=str(day), callback_data=f'day:{year}:{month}:{day}'
            ))
...
def generate_final_inline_kb():
    ...
    final_inline_kb.row(InlineKeyboardButton(text=f'{str(Emoji.Cancel)} Отмена', callback_data='cancel_all'))

    return final_inline_kb.as_markup()


def generate_fill_manual_kb():
...
def generate_cancel_inline_kb():
    ...
    cancel_inline_kb.button(text=f'{str(Emoji.Cancel)} Отмена', callback_data='cancel_all')
